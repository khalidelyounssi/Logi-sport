            ...
            <div class="border-b border-slate-100 bg-slate-50 px-6 py-5">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <h2 class="mt-1 text-xl font-black text-slate-900">{{ $match->participantA?->name }} vs {{ $match->participantB?->name }}</h2>
                    </div>
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div class="rounded-3xl border border-slate-200 bg-white p-6 text-center shadow-sm">
                        ...
                        <div class="mt-6 flex items-center justify-center gap-6">
                            <button type="button" onclick="decreaseScore('score_a', 'score_a_display')" class="inline-flex h-12 w-12 items-center justify-center rounded-full border border-blue-200 text-2xl font-bold text-blue-600 transition hover:bg-blue-50">
                                -
                            </button>
                            <span id="score_a_display" class="text-6xl font-black text-blue-700">
                                {{ old('score_a', $match->score_a ?? 0) }}
                            </span>
                            <button type="button" onclick="increaseScore('score_a', 'score_a_display')" class="inline-flex h-12 w-12 items-center justify-center rounded-full bg-blue-600 text-2xl font-bold text-white transition hover:bg-blue-700">
                                +
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-white p-6 text-center shadow-sm">
                        ...
                        <div class="mt-6 flex items-center justify-center gap-6">
                            <button type="button" onclick="decreaseScore('score_b', 'score_b_display')" class="inline-flex h-12 w-12 items-center justify-center rounded-full border border-blue-200 text-2xl font-bold text-blue-600 transition hover:bg-blue-50">
                                -
                            </button>
                            <span id="score_b_display" class="text-6xl font-black text-blue-700">
                                {{ old('score_b', $match->score_b ?? 0) }}
                            </span>
                            <button type="button" onclick="increaseScore('score_b', 'score_b_display')" class="inline-flex h-12 w-12 items-center justify-center rounded-full bg-blue-600 text-2xl font-bold text-white transition hover:bg-blue-700">
                                +
                            </button>
                        </div>
                    </div>
        ...
        <x-ui.card>
            ...
            <div class="mt-4 space-y-4 text-sm">
                ...
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
                    ...
                    <p class="mt-1 font-semibold text-slate-800">{{ $match->match_date?->format('Y-m-d H:i') ?? 'Not scheduled' }}</p>
                </div>
                <div class="rounded-2xl bg-slate-50 px-4 py-3">
