<x-app-layout>
    <x-slot name="title">Create New Sport</x-slot>
    <x-slot name="subtitle">Add a new sport to the system</x-slot>
            ...
            <form action="{{ route('sports.store') }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="rules" name="rules" rows="4" placeholder="Enter sport rules or description..."
                        class="w-full px-4 py-2 bg-slate-800 border border-slate-700 rounded-lg text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:border-emerald-500 focus:ring-emerald-500 @error('rules') ring-1 ring-red-500 @enderror">{{ old('rules') }}</textarea>
                    @error('rules')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
