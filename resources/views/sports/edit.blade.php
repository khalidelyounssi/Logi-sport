            ...
            <form action="{{ route('sports.update', $sport) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="rules" name="rules" rows="4" placeholder="Enter sport rules or description..."
                        class="w-full px-4 py-2 bg-slate-800 border rounded-lg text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 @error('rules') border-red-500 @else border-slate-700 @enderror focus:border-emerald-500 focus:ring-emerald-500">{{ old('rules', $sport->rules) }}</textarea>
                    @error('rules')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
